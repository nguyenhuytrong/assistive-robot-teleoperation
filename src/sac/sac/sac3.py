...
class SharedAutonomyController(Node):
    def __init__(self):
        ...
        #self.declare_parameter('output_joy_topic', 'sac/cmd_joy')
        ...
        self.declare_parameter('rho_cap', 0.5)
    ...
    def joy_bs_callback(self, joy_bs_msg: Joy):
        ...
        if len(joy_bs_msg.buttons) > 6:
            self.deadman_switch = joy_bs_msg.buttons[6]
        else:
            self.deadman_switch = 0
    ...
    def publish_finalVelocity_marker(self):
        # Always pass human input through, blend with repulsion
        self.vfinal_signal = self.compute_resultant([self.ref_signal, self.rep_resultant])

        # No normalization — let full resultant through
        # (signals are already in joystick range -1.0 to 1.0)
        normFactor2 = NORM_FACTOR_2  # = 1

        self.vfinal_signal.x /= normFactor2
        self.vfinal_signal.y /= normFactor2
        self.vfinal_signal.z /= normFactor2

        # Clamp to joystick range [-1, 1]
